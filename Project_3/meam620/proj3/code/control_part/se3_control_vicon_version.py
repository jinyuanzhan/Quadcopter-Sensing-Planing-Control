...
class SE3Control(object):
    ...
    def __init__(self, quad_params):
        ...
        # self.k_p = np.diag([20, 37, 80])
        # self.k_d = np.diag([9.6, 10.3, 800])
        # # self.k_i = np.diag([25., 25., 25.0])
        # self.k_R = np.diag([5500, 5500, 500])  # the thir parameter in K_R is used to tune the yaw angle #like P here # roll pich yaw
        # self.k_w = np.diag([100, 120, 130]) # like D here

        self.k_p = np.diag([3.75, 3.75, 30])
        self.k_d = np.diag([3.25, 3.25, 8.25])
    ...
    def update(self, t, state, flat_output):
        ...
        x_ddot_des = x_ddot_target - self.k_d @ (x_dot_current - x_dot_target) - self.k_p @ (x_current - x_target)  

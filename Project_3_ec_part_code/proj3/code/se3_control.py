...
class SE3Control(object):
    ...
    def __init__(self, quad_params):
        ...
        # self.k_p = np.diag([8, 8, 80])  # 
        # self.k_d = np.diag([5.5, 5.25, 50])   
        # self.k_i = np.diag([3.5, 3.5, 3.5])  
        # self.k_R = np.diag([5500, 5500, 500]) 
        # self.k_w = np.diag([120, 120, 120])  # 
        
        self.k_p = np.diag([23, 23, 26] )* 0.805 # fixed for over under
        self.k_d = np.diag([8,8,8.5]) * 0.35 
        self.k_i = np.diag([5, 5, 6])  
        self.k_R = np.diag([5500, 5500,300])* 1  
        self.k_w = np.diag([130, 130, 130])* 1.03  
    ...
    def update(self, t, state, flat_output):
        ...
        u_matrix = np.vstack((u1.reshape(1, 1), u2.reshape(3, 1)))  # 4x1 矩阵 [F, M_x, M_y, M_z]^T
